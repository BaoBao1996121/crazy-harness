...
| 6 | [Composite Checkpoint](COMPOSITE_CHECKPOINT_DESIGN.md) | 如何冻结 Workspace、状态引用、Artifact 和 Effect 边界，并从同一请求跨崩溃 Fork Restore？ |
| 7 | [Harness 核心机制](HARNESS_CORE_ESSENTIALS.md) | Loop、Context、Memory、Tool、A2A、Eval 各自控制什么？ |
| 8 | [通用 Agent Team 主计划](GENERAL_AGENT_TEAM_MASTER_PLAN.md) | 当前实现位于哪一阶段，如何继续演进为通用平台？ |
| 9 | [Evidence Research TaskPack](EVIDENCE_RESEARCH_TASKPACK.md) | 同一 Runtime 如何换业务壳，并用浏览器证据、引用和 Hash 门禁准出？ |
| 10 | [Agent Skills 渐进披露](AGENT_SKILLS_PROGRESSIVE_DISCLOSURE_WALKTHROUGH.md) | metadata、正文和资源如何按需进入 Context？ |
| 11 | [MCP 延迟发现](MCP_DELAYED_DISCOVERY_WALKTHROUGH.md) | 远端工具如何授权挂载、搜索、披露和执行？ |
| 12 | [16 小时实码学习手册](HARNESS_16H_ACTUAL_CODE_LEARNING_GUIDE.md) | 如何从最小 Loop 调试到 Context、A2A、Memory 与 Eval？ |
| 13 | [第三方许可证矩阵](THIRD_PARTY_LICENSE_MATRIX.md) | 哪些是协议兼容、模式借鉴或可选依赖？ |
| 14 | [Resident A2A v0.1 历史 Checkpoint](RESIDENT_A2A_V01_CHECKPOINT.md) | 最初 MVP 如何定界？仅用于理解演进历史，不代表当前实现。 |
