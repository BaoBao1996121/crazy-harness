...
| FastAPI / Uvicorn | Existing project dependencies; used by AgentRun control API | Six run-control/query routes and health `v0.10.0-dev` verified locally | Available |
| React / TypeScript toolchain | Existing `frontend/` lockfile; used by Control Room | Full Vitest `23 files / 89 tests passed` and production build passed | Available |
...
| Browser / Chromium | Control Room visual verification | AgentRun controls verified after PID `34028` restart at 1440x900 and 390x844 with no console logs or horizontal overflow | Available for local verification |
...
The reproducible records are the three `work/spikes/verify_engineering_loop_*.py` scripts and the matching entry in `docs/DEVELOPMENT_JOURNAL.md`:
