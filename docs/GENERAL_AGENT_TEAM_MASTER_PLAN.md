...
> 交付节奏：纵向能力探索优先，验证按 Smoke / Core / Release / Nightly 分层；关键权限、持久化和副作用边界仍须随改随测，低风险全量回归不阻塞每个探索切片。
...
结论：**质量门不取消，而是按风险和时机分层执行。** 开发中的最小纵向切片先证明真实可运行；关键不变量即时验证；耗时、跨平台和概率性回归后移到 PR、里程碑或定时任务。

| Stage | 何时运行 | 验证内容 | 失败后的处理 |
|---|---|---|---|
| Changed | 每次行为修改后 | 本次变更的 RED/GREEN 测试与最窄邻接用例 | 不得继续叠加同一机制 |
| Smoke | 一个纵向切片可运行后，目标小于 1 分钟 | 启动、提交任务、关键状态转移、核心安全边界 | 修复后再演示或进入下一切片 |
| Core | 一个机制或 Sprint 收口时 | 确定性 Agent Loop、Kernel、Mailbox、Context、Tool、Team 主回归 | 不得宣称该机制完成 |
| Release | PR / 版本候选 | Core + Ruff + 前端测试/构建 + Linux/Windows | 不得合并或发布 |
| Nightly | 定时或重要架构变更后 | 压力、Chaos、重复采样、真实 LLM、外部 Adapter | 形成缺陷或 Eval 报告，不阻塞白天探索，除非触发安全硬门禁 |

分层不是按“测试重要不重要”划分，而是按反馈时效和成本划分。鉴权、Command Validation、Fencing、CompletionGate、不可逆副作用与终态不变量属于硬边界，即使在 Changed/Smoke 阶段也不能跳过；跨平台全量组合、长时间压力和多次模型采样则进入更晚阶段。

阶段突破另记入 `DEVELOPMENT_JOURNAL.md`。只要切片涉及前端，完成证据至少包含一张能实际解码打开、能辨认本次变化的桌面截图；响应式布局变化通常再补 390px 移动端截图，并记录 Run ID、视口和控制台结果。纯后端阶段使用最窄复现命令或持久 Event 作证，不为了截图制造无关 UI。
...
当前 Team Worker 支持两种 Model Provider：`scripted` 用于确定性 Replay 与故障注入，`deepseek` 用于真实在线动作；二者共用同一个 canonical AgentLoop、Command Validation、ToolPipeline 与 CompletionGate。v0.6 Scheduler 已支持两路真实受控并发、持久背压、跨 Runtime Claim、续期、Fenced Commit/Ack 与协作取消；v0.7 又为 Assignment/Peer child AgentRun 增加独立模型路由和持久预算治理。A2A 仍经过本地 SQLite EventStore 与 Durable Mailbox，Remote A2A 和分布式公平调度不能从这张图推断为已经完成。
...
v0.6 受控并发纵切已经完成。TeamContract 现在包含 evidence 与 risk 两个独立根阶段，二者真实重叠执行，再由 artifact join 和 review 收尾。Scheduler 使用有界线程池、全局与单 Worker 容量、Agent 间 Round-Robin 和持久 Mailbox 背压；每次 Dispatch 在 SQLite 中原子认领 Delivery + AgentRun + Worker Slot 三个 key，由独立 Renewer 延长 TTL。单调 fencing token 不只守住 Kernel 正式提交和 Mailbox Ack，也在同一 SQLite 事务中守住 DispatchContext 内的普通可信 Event append；旧 Worker 只能由 Scheduler 在 context 外留下 `claim.lost` 诊断。一次瞬时续租故障会重试，接近安全期限仍无法确认时释放但不 Ack。Run 取消会停止新准入、取消排队 Delivery，并通过 DispatchContext 向在途 Handler 和 ToolPipeline 传播协作式 CancellationToken。Run、Assignment 与 Lease Projection 均采用吸收终态，清空后重放不会被迟到事件倒退。
...
v0.8e 已把单 Pair 扩展为持久 Eval Campaign：父 Contract 预注册 Trial，约束 `2 x Trial x 每臂预算` 与 Pair 并发窗口；每个 Pair 仍走相同 Prepare -> Commit -> Link -> Release 协议，父级保存 Pair Report Hash 并按固定 Seed 做配对聚合。容量暂时耗尽进入持久 Wait，Lease 释放后用确定性 Nudge 恢复；取消可回收 Pair Contract 前的孤立 Arm，scoped drain 会先建立失败/取消写屏障。Pair Create Claim 在长 Prepare 期间续租；暂时失败保持原身份重试；Scorer/Aggregator 版本漂移都生成不可晋升的终态证据。实现与边界见 `PAIRED_EVAL_CAMPAIGN_DESIGN.md`。

尚未完成：本机没有 `DEEPSEEK_API_KEY`，所以 Team 的付费在线 Smoke 和至少 10 个 `live_paired` Trial 仍是外部门槛；当前三轮 Scripted Golden Campaign 只得到点估计，可否决当前 Case 的耗时退化但不能晋升 Team。Remote A2A Adapter、真正的数据库通知/消息代理和分布式公平性也仍待后续纵切。当前共享 SQLite 通过最多 1 秒的有界检查发现其他进程写入或 Claim 到期，这保证本地多 Runtime 活性但不是远程 Broker；Round-Robin cursor 仍是进程内事实。外部 HTTP 副作用也不能由 Fencing 撤回，仍依赖 OperationLedger、业务幂等键、对账或补偿。Assignment Lease 使用 at-least-once 恢复语义，不能宣称分布式 exactly-once；每个 Scheduler 进程总并发 `2`、跨 Scheduler 单 Agent 持久并发 `1`、Work Claim TTL `180s`、外部写入检查 `1s`、Team 最大并行 Assignment `2` 与 Stage Attempt `3` 都是初始配置值，待真实任务 Eval 和压力数据调优。当前还没有跨进程共享 Global Slot，因此多个 Scheduler 处理不同 Agent 时的总并发可超过单进程 `2`。
