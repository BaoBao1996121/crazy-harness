...
const EXACT: Record<string, Partial<EventMeta> & Pick<EventMeta, "label">> = {
  ...
  "orchestration.blocked": { label: "编排阻塞 / Orchestration blocked", family: "a2a", boundary: "control", tone: "danger" },
  "orchestration.capacity.waiting": { label: "等待 Agent 容量 / Waiting for agent capacity", family: "a2a", boundary: "runtime", tone: "warning", important: true },
  "orchestration.capacity.resumed": { label: "Agent 容量恢复 / Agent capacity restored", family: "a2a", boundary: "fact", tone: "success", important: true },
  "eval.arm.planned": { label: "评测臂已规划 / Eval arm planned", family: "runtime", boundary: "fact" },
  "eval.arm.created": { label: "评测臂已创建 / Eval arm created", family: "runtime", boundary: "fact" },
  "eval.arm.linked": { label: "评测臂已关联 / Eval arm linked", family: "runtime", boundary: "fact" },
  "eval.arm.released": { label: "评测臂已释放执行 / Eval arm released", family: "runtime", boundary: "fact", tone: "info" },
  "eval.arm.cleanup.failed": { label: "评测臂清理失败 / Eval arm cleanup failed", family: "runtime", boundary: "fact", tone: "danger", important: true },
  "eval.pair.requested": { label: "配对评测已请求 / Eval pair requested", family: "gate", boundary: "control" },
  "eval.pair.created": { label: "配对评测已创建 / Eval pair created", family: "gate", boundary: "fact" },
  "eval.pair.committed": { label: "配对契约已提交 / Eval pair committed", family: "gate", boundary: "fact", tone: "success", important: true },
  "eval.pair.completed": { label: "配对评测已完成 / Eval pair completed", family: "gate", boundary: "fact", tone: "success", important: true },
  "eval.pair.failed": { label: "配对评测创建失败 / Eval pair failed", family: "gate", boundary: "fact", tone: "danger", important: true },
  "eval.pair.finalization.failed": { label: "配对评测收尾失败 / Eval pair finalization failed", family: "gate", boundary: "fact", tone: "danger", important: true },
  "eval.pair.recovery.failed": { label: "配对评测恢复失败 / Eval pair recovery failed", family: "gate", boundary: "fact", tone: "danger", important: true },
  "eval.campaign.requested": { label: "多轮评测已请求 / Campaign requested", family: "gate", boundary: "control" },
  "eval.campaign.created": { label: "多轮评测已创建 / Campaign created", family: "gate", boundary: "fact", important: true },
  "eval.campaign.scope.bound": { label: "Campaign 评测范围已冻结 / Campaign scope bound", family: "gate", boundary: "control" },
  "eval.campaign.trial.started": { label: "Trial 已启动 / Campaign trial started", family: "gate", boundary: "fact" },
  "eval.campaign.trial.linked": { label: "Trial 已关联 Pair / Campaign trial linked", family: "gate", boundary: "fact" },
  "eval.campaign.trial.released": { label: "Trial 已释放执行 / Campaign trial released", family: "gate", boundary: "fact", tone: "info" },
  "eval.campaign.trial.observed": { label: "Trial 证据已观察 / Campaign trial observed", family: "gate", boundary: "fact", tone: "success", important: true },
  "eval.campaign.trial.creation_retryable_failed": { label: "Trial 创建暂时失败，等待重试 / Trial creation retry pending", family: "gate", boundary: "runtime", tone: "warning", important: true },
  "eval.campaign.trial.creation_failed": { label: "Trial 创建失败 / Campaign trial creation failed", family: "gate", boundary: "fact", tone: "danger", important: true },
  "eval.campaign.completed": { label: "多轮评测已完成 / Campaign completed", family: "gate", boundary: "fact", tone: "success", important: true },
  "eval.campaign.cancelled": { label: "多轮评测已取消 / Campaign cancelled", family: "gate", boundary: "fact", tone: "warning", important: true },
  ...
  "assignment.expired": { label: "任务租约到期 / Assignment expired", family: "a2a", boundary: "fact", tone: "danger" },
  "assignment.lease.acquired": { label: "租约获取 / Lease acquired", family: "a2a", boundary: "fact", tone: "info" },
  "assignment.lease.renewed": { label: "租约续期 / Lease renewed", family: "a2a", boundary: "runtime" },
  "assignment.lease.released": { label: "租约释放 / Lease released", family: "a2a", boundary: "fact", tone: "success" },
  "assignment.lease.expired": { label: "租约超时 / Lease expired", family: "a2a", boundary: "fact", tone: "danger" },
  "assignment.delivery.stale": { label: "过期投递已隔离 / Stale delivery fenced", family: "a2a", boundary: "control", tone: "warning" },
  "agent.result.submitted": { label: "Agent 结果提交 / Agent result submitted", family: "a2a", boundary: "fact" },
  "agent.run.created": { label: "子 AgentRun 创建 / Child AgentRun created", family: "a2a", boundary: "fact", tone: "info" },
  "agent.waiting": { label: "Agent 等待外部事件 / Agent waiting", family: "loop", boundary: "fact", tone: "warning" },
  "agent.action.denied": { label: "Agent 动作被拒 / Agent action denied", family: "gate", boundary: "control", tone: "danger" },
  "agent.result.promoted": { label: "结果晋升为团队事实 / Result promoted", family: "a2a", boundary: "fact", tone: "success" },
  "agent.result.rejected": { label: "结果晋升被拒 / Result promotion rejected", family: "a2a", boundary: "control", tone: "danger" },
  "a2a.message.sent": { label: "A2A 消息已发出 / A2A message sent", family: "a2a", boundary: "fact", tone: "info" },
  "a2a.message.denied": { label: "A2A 消息被拒 / A2A message denied", family: "a2a", boundary: "control", tone: "danger" },
  ...
  "run.cancel.requested": { label: "运行取消请求 / Run cancellation requested", family: "runtime", boundary: "control", tone: "warning", important: true },
  "run.cancelled": { label: "运行已取消 / Run cancelled", family: "runtime", boundary: "fact", tone: "warning", important: true },
  "assignment.cancelled": { label: "任务已取消 / Assignment cancelled", family: "a2a", boundary: "fact", tone: "warning", important: true },
  "runtime.agent.crashed": { label: "执行中断 / Worker crashed", family: "runtime", boundary: "runtime", tone: "danger" },
  "runtime.agent.degraded": { label: "Agent 已降级 / Agent degraded", family: "runtime", boundary: "fact", tone: "danger" },
  "runtime.agent.heartbeat": { label: "Agent 心跳 / Agent heartbeat", family: "runtime", boundary: "runtime" },
  ...
  "runtime.scheduler.cycle.failed": { label: "调度周期故障，正在恢复 / Scheduler cycle recovering", family: "runtime", boundary: "runtime", tone: "danger" },
  "runtime.delivery.dispatched": { label: "投递已派发 / Delivery dispatched", family: "runtime", boundary: "runtime", tone: "info" },
  "runtime.scheduler.backpressure": { label: "调度器背压 / Scheduler backpressure", family: "runtime", boundary: "runtime", tone: "warning", important: true },
  "runtime.delivery.claim.acquired": { label: "工作声明获取 / Work claim acquired", family: "runtime", boundary: "fact", tone: "info" },
  "runtime.delivery.claim.renewed": { label: "工作声明续期 / Work claim renewed", family: "runtime", boundary: "runtime" },
  "runtime.delivery.claim.released": { label: "工作声明释放 / Work claim released", family: "runtime", boundary: "fact", tone: "success" },
  "runtime.delivery.claim.lost": { label: "工作声明丢失 / Work claim lost", family: "runtime", boundary: "fact", tone: "danger", important: true },
  "runtime.delivery.cancellation.requested": { label: "投递取消请求 / Delivery cancellation requested", family: "runtime", boundary: "control", tone: "warning", important: true },
  "runtime.delivery.cancelled": { label: "投递已取消 / Delivery cancelled", family: "runtime", boundary: "fact", tone: "warning", important: true },
  ...
  "mailbox.delivery.dead_lettered": { label: "邮箱消息进入死信 / Delivery dead-lettered", family: "runtime", boundary: "fact", tone: "danger" },
  "a2a.peer.failure.unroutable": { label: "同伴失败无法路由 / Peer failure unroutable", family: "a2a", boundary: "fact", tone: "danger" },
...
const NOISE = new Set([
  ...
  "runtime.delivery.claim.renewed",
