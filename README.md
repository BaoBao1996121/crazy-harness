...
- `Agent Team 演示`：Supervisor 根据持久 TeamContract、AgentCard、状态与负载逐阶段生成 PlanPatch；Worker 当前仍为 Scripted 基线。
...
- Team v0.4 的 Supervisor 与 Lease 可真实运行，但 Worker 仍使用 Scripted 策略，Scheduler 仍是单消费者；真实并行、Remote A2A Adapter 与 Single-vs-Team 收益评测尚未完成。
