...
- Team v0.5 的 Supervisor、Lease、canonical Team Worker AgentLoop、原子 Command 与 Event Outbox 已可真实运行，但 Team 的模型提供器仍是 Scripted Model；在线 DeepSeek Team、真实并行执行、Remote A2A Adapter 与 Single-vs-Team 收益评测尚未完成。Scheduler 仍是单消费者，本地 A2A 仍依赖同进程 EventStore 与 Durable Mailbox。
